...
import { LogOut, Plus, Pencil, Trash2, RotateCcw, Lock, Shield, Download, Loader2, AlertCircle, Search, XCircle, Mail, Award, Users, Database, Sparkles, FileDown, Calendar, FileText, Eye, CheckCircle, Clock, ExternalLink } from "lucide-react";
import { MemberCard } from "@/components/MemberCard";
import { processAndWatermark } from "@/lib/imageUtils";
import type { Opportunity, Partner, Training, NewsArticle, Event, Achievement, Member, ContactMessage, NewsletterSubscriber, ThunderbirdApplication, ElectionCandidate, AwsRestartApplication, PlanActivity, ActivityReport } from "@shared/schema";
...
// ─── Onglet Admin : Calendrier des Activités ──────────────────────────────────
function PlanActivitiesAdminTab() {
  const { toast } = useToast();
  const [addOpen, setAddOpen] = useState(false);
  const [editOpen, setEditOpen] = useState(false);
  const [editItem, setEditItem] = useState<PlanActivity | null>(null);
  const [form, setForm] = useState({ month: "", title: "", type: "Forum", description: "", date: "", target: "", documentUrl: "", sortOrder: "99", isActive: true });

  const resetForm = () => setForm({ month: "", title: "", type: "Forum", description: "", date: "", target: "", documentUrl: "", sortOrder: "99", isActive: true });

  const { data: activities = [], isLoading } = useQuery<PlanActivity[]>({
    queryKey: ["/api/admin", "plan-activities"],
    queryFn: async () => {
      const res = await fetch("/api/admin/plan-activities", { credentials: "include" });
      if (!res.ok) throw new Error("Failed to fetch");
      return res.json();
    },
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin", "plan-activities"] });
      setAddOpen(false);
      resetForm();
      toast({ title: "Activité créée avec succès" });
    },
    onError: (err: any) => toast({ title: "Erreur", description: err.error || "Erreur", variant: "destructive" }),
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => {
      const res = await fetch(`/api/admin/plan-activities/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify(data),
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      await fetch(`/api/admin/plan-activities/${id}`, { method: "DELETE", credentials: "include" });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin", "plan-activities"] });
  ...
  const openEdit = (item: PlanActivity) => {
    setEditItem(item);
    setForm({
      month: item.month,
      title: item.title,
  ...
  const ActivityForm = ({ isEdit }: { isEdit: boolean }) => (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          <Label>Mois *</Label>
          <Input placeholder="ex: Juillet 2026" value={form.month} onChange={e => setForm({ ...form, month: e.target.value })} />
        </div>
        <div className="space-y-1">
          <Label>Type *</Label>
      ...
      <div className="space-y-1">
        ...
        <Input placeholder="Ex: Forum sur l'Entrepreneuriat Numérique" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} />
      </div>
      <div className="space-y-1">
        <Label>Description *</Label>
        <Textarea rows={3} placeholder="Décrivez l'activité..." value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          <Label>Date / Période *</Label>
          <Input placeholder="ex: 10-12 Juillet 2026" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} />
        </div>
        <div className="space-y-1">
          <Label>Audience Cible *</Label>
          <Input placeholder="ex: Étudiants en informatique" value={form.target} onChange={e => setForm({ ...form, target: e.target.value })} />
        </div>
      </div>
      <div className="space-y-1">
        <Label>Lien document (PDF / Drive)</Label>
        <Input type="url" placeholder="https://drive.google.com/..." value={form.documentUrl} onChange={e => setForm({ ...form, documentUrl: e.target.value })} />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
      ...
      <Button className="w-full" onClick={() => handleSubmit(isEdit)} disabled={createMutation.isPending || updateMutation.isPending}>
        {createMutation.isPending || updateMutation.isPending ? "Enregistrement..." : isEdit ? "Mettre à jour" : "Créer l'activité"}
      </Button>
    </div>
  );
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h2 className="text-lg font-semibold flex items-center gap-2">
            ...
            Calendrier des Activités ({activities.length})
          </h2>
          <p className="text-xs text-muted-foreground">Gérez les activités affichées sur la page "Plan Semestriel".</p>
        </div>
        <Dialog open={addOpen} onOpenChange={v => { setAddOpen(v); if (!v) resetForm(); }}>
          <DialogTrigger asChild>
            <Button className="bg-green-600 hover:bg-green-700"><Plus className="h-4 w-4 mr-2" />Ajouter une activité</Button>
          </DialogTrigger>
          <DialogContent className="max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Nouvelle activité au calendrier</DialogTitle>
              <DialogDescription>Remplissez les informations pour ajouter une activité au plan semestriel.</DialogDescription>
            </DialogHeader>
      ...
      <Dialog open={editOpen} onOpenChange={v => { setEditOpen(v); if (!v) setEditItem(null); }}>
        <DialogContent className="max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Modifiez les informations de cette activité du calendrier.</DialogDescription>
          </DialogHeader>
            ...
            {activities.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="text-center text-muted-foreground py-12">
                  ...
                  <p>Aucune activité. Cliquez sur "Ajouter" pour créer la première entrée du calendrier.</p>
                </TableCell>
              </TableRow>
            ) : (
              activities.map((act) => (
                <TableRow key={act.id}>
                  <TableCell className="font-medium text-sm">{act.month}</TableCell>
                  <TableCell className="max-w-[180px] truncate font-medium">{act.title}</TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    {act.documentUrl ? (
                      <a href={act.documentUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex items-center gap-1 text-xs">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <Switch
                        ...
                        onCheckedChange={(checked) => toggleActive(act.id, checked)}
                        disabled={updateMutation.isPending}
                      />
                      <Badge variant={act.isActive ? "default" : "secondary"} className="text-xs">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      <Button size="icon" variant="ghost" onClick={() => openEdit(act)}>
                        <Pencil className="h-4 w-4" />
                      </Button>
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              <strong>"{act.title}"</strong> sera définitivement supprimée du calendrier.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>Annuler</AlertDialogCancel>
                            <AlertDialogAction onClick={() => deleteMutation.mutate(act.id)} className="bg-destructive text-destructive-foreground">
                              Supprimer
                            </AlertDialogAction>
                          </AlertDialogFooter>
                        </AlertDialogContent>
...
// ─── Onglet Admin : Rapports Soumis ──────────────────────────────────────────
function ActivityReportsAdminTab() {
  const { toast } = useToast();

  const { data: reports = [], isLoading, refetch } = useQuery<any[]>({
    queryKey: ["/api/admin", "activity-reports"],
    queryFn: async () => {
      const res = await fetch("/api/admin/activity-reports", { credentials: "include" });
      if (!res.ok) throw new Error("Failed to fetch");
      return res.json();
    },
  });
  ...
  const markReviewedMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`/api/admin/activity-reports/${id}/status`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin", "activity-reports"] });
      toast({ title: "Marqué comme examiné" });
    },
    onError: () => toast({ title: "Erreur", variant: "destructive" }),
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      await fetch(`/api/admin/activity-reports/${id}`, { method: "DELETE", credentials: "include" });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin", "activity-reports"] });
  ...
  const typeLabels: Record<string, string> = {
    ...
    event: "Résumé d'événement",
    governance: "Note de Gouvernance",
  };
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h2 className="text-lg font-semibold flex items-center gap-2">
            ...
            Rapports Soumis ({reports.length})
            {pending > 0 && (
          ...
          <p className="text-xs text-muted-foreground">Rapports et documents transmis via le formulaire "Soumettre un Rapport" du site.</p>
        </div>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {reports.length === 0 ? (
              ...
              reports.map((report) => (
                <TableRow key={report.id} className={report.status === "pending" ? "bg-orange-50/40 dark:bg-orange-950/10" : ""}>
                  <TableCell className="font-medium max-w-[160px] truncate">{report.title}</TableCell>
                  <TableCell>
                    <Badge variant="outline" className="text-xs">{typeLabels[report.type] || report.type}</Badge>
                  </TableCell>
                  <TableCell className="text-sm text-muted-foreground">{report.submittedBy || "Anonyme"}</TableCell>
                  <TableCell>
                    {report.hasFile ? (
                      <a
                        href={`/api/admin/activity-reports/${report.id}/file`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-1 text-blue-600 hover:underline text-xs"
                      >
                        <Download className="h-3.5 w-3.5" />
                        {report.fileName ? report.fileName.substring(0, 20) : "Télécharger"}
                      </a>
                    ) : (
                      <span className="text-muted-foreground text-xs">Pas de fichier</span>
                    )}
                  </TableCell>
                  <TableCell>
                    <Badge variant={report.status === "reviewed" ? "default" : "secondary"}
                      className={report.status === "pending" ? "bg-orange-100 text-orange-800 border-orange-200" : ""}>
                      {report.status === "reviewed" ? (
                        <><CheckCircle className="h-3 w-3 mr-1" />Examiné</>
                      ) : (
                      ...
                      <Dialog>
                        ...
                        <DialogContent className="max-w-xl max-h-[85vh] overflow-y-auto">
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              {typeLabels[report.type] || report.type} — soumis le {report.createdAt ? new Date(report.createdAt).toLocaleDateString("fr-FR") : ""}
                            </DialogDescription>
                          </DialogHeader>
                          <div className="space-y-4 py-2">
                            <div className="space-y-1">
                              <Label className="text-xs text-muted-foreground">Soumis par</Label>
                              <p className="text-sm">{report.submittedBy || "Anonyme"}</p>
                            </div>
                            <div className="space-y-1">
                              <Label className="text-xs text-muted-foreground">Description / Résumé</Label>
                              <div className="text-sm bg-muted/30 p-4 rounded-lg border whitespace-pre-wrap leading-relaxed">
                                {report.description}
                              </div>
                            </div>
                            {report.hasFile && (
                              <div className="space-y-1">
                                <Label className="text-xs text-muted-foreground">Fichier joint</Label>
                                <a
                                  href={`/api/admin/activity-reports/${report.id}/file`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                ...
                                >
                                  ...
                                  {report.fileName} {report.fileSize ? `(${(report.fileSize / 1024 / 1024).toFixed(2)} MB)` : ""}
                                </a>
                              </div>
                            )}
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Le rapport <strong>"{report.title}"</strong> et son fichier associé seront définitivement supprimés.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>Annuler</AlertDialogCancel>
        ...
        <Tabs defaultValue="opportunities">
          <div className="overflow-x-auto pb-2 -mx-4 px-4 md:mx-0 md:px-0 scrollbar-hide">
            <TabsList className="flex min-w-max md:min-w-0 md:flex-wrap gap-1 h-auto mb-2 md:mb-6 bg-transparent border-none p-0">
              ...
              <TabsTrigger value="plan-activities" className="border text-green-700 font-bold"><Calendar className="h-3.5 w-3.5 mr-1" />Calendrier</TabsTrigger>
              <TabsTrigger value="activity-reports" className="border text-purple-600 font-bold"><FileText className="h-3.5 w-3.5 mr-1" />Rapports</TabsTrigger>
            </TabsList>
          </div>
          ...
          <TabsContent value="plan-activities">
            <PlanActivitiesAdminTab />
          </TabsContent>
          <TabsContent value="activity-reports">
            <ActivityReportsAdminTab />
          </TabsContent>
        </Tabs>
      </div>
    </div>
