...
import {
  ...
  type PlanActivity, type InsertPlanActivity,
  type ActivityReport, type InsertActivityReport,
  ...
  awsRestartApplications, planActivities, activityReports
} from "../shared/schema";
...
export interface IStorage {
  ...
  createPlanActivity(activity: InsertPlanActivity): Promise<PlanActivity>;
  getAllPlanActivities(): Promise<PlanActivity[]>;
  getActivePlanActivities(): Promise<PlanActivity[]>;
  updatePlanActivity(id: string, data: Partial<InsertPlanActivity>): Promise<PlanActivity | undefined>;
  deletePlanActivity(id: string): Promise<void>;

  // Activity Reports (rapports soumis)
  createActivityReport(report: InsertActivityReport): Promise<ActivityReport>;
  getAllActivityReports(): Promise<ActivityReport[]>;
  updateActivityReportStatus(id: string, status: string): Promise<ActivityReport | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  // ─── Plan Activities ────────────────────────────────────────────────────────
  async createPlanActivity(activity: InsertPlanActivity): Promise<PlanActivity> {
    const [newActivity] = await db.insert(planActivities).values(activity).returning();
    return newActivity;
  }

  async getAllPlanActivities(): Promise<PlanActivity[]> {
    return db.select().from(planActivities).orderBy(asc(planActivities.sortOrder), asc(planActivities.createdAt));
  }

  async getActivePlanActivities(): Promise<PlanActivity[]> {
    return db.select().from(planActivities)
      .where(eq(planActivities.isActive, true))
      .orderBy(asc(planActivities.sortOrder), asc(planActivities.createdAt));
  }

  async updatePlanActivity(id: string, data: Partial<InsertPlanActivity>): Promise<PlanActivity | undefined> {
    const [updated] = await db.update(planActivities).set(data).where(eq(planActivities.id, id)).returning();
    return updated;
  }

  async deletePlanActivity(id: string): Promise<void> {
  ...
  async createActivityReport(report: InsertActivityReport): Promise<ActivityReport> {
    const [newReport] = await db.insert(activityReports).values(report).returning();
    return newReport;
  }

  async getAllActivityReports(): Promise<ActivityReport[]> {
    return db.select().from(activityReports).orderBy(desc(activityReports.createdAt));
  }

  async updateActivityReportStatus(id: string, status: string): Promise<ActivityReport | undefined> {
    const [updated] = await db.update(activityReports).set({ status }).where(eq(activityReports.id, id)).returning();
    return updated;
  }
