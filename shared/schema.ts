...
// ─── Plan Semestriel des Activités ────────────────────────────────────────────
export const planActivities = pgTable("plan_activities", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  month: text("month").notNull(),          // ex: "Juillet 2025"
  title: text("title").notNull(),
  type: text("type").notNull(),            // Forum | Hackathon | Bootcamp | Projet
  description: text("description").notNull(),
  date: text("date").notNull(),            // ex: "10-12 Juillet 2025"
  target: text("target").notNull(),        // Audience cible
  documentUrl: text("document_url"),       // Lien PDF / Google Drive
  sortOrder: integer("sort_order").notNull().default(99),
  isActive: boolean("is_active").notNull().default(true),
  createdAt: timestamp("created_at").defaultNow(),
});
...
export type InsertPlanActivity = z.infer<typeof insertPlanActivitySchema>;
export type PlanActivity = typeof planActivities.$inferSelect;

// ─── Rapports / Documents soumis via le formulaire public ─────────────────────
export const activityReports = pgTable("activity_reports", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  title: text("title").notNull(),
  type: text("type").notNull(),            // activity | project | event | governance
  description: text("description").notNull(),
  fileName: text("file_name"),             // Nom du fichier uploadé
  fileData: text("file_data"),             // Contenu base64 du fichier
  fileSize: integer("file_size"),          // Taille en octets
  submittedBy: text("submitted_by"),       // Email de la personne (optionnel)
